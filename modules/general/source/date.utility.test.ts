...
import { getCartesianProduct, getCollectionRepresentation as $c } from "./collection.utility";
import { getSafeDate, isDate, isDateBetween, isDateCompatible, isDateRange, ISO_DATE_PATTERN } from "./date.utility";
import { getTypeRepresentation as $t } from "./type";
...
describe("Date Utility", () => {
  ...
  describe("#isDateCompatible", () => {
    ...
    getCartesianProduct(dateValues, dateValues).forEach(dateParameters => {
      ...
      it(`should support ${$t(parameter1)} & ${$t(parameter2)} parameters`, () => {
    ...
    getCartesianProduct(date1Values, date2Values).forEach(dateRangeParameter => {
      ...
      dateValues.forEach(dateValue => {
        it(`should support ${$t(dateValue)}\` & [${$t(dateRangeLowBoundary)}:${$t(
  ...
  describe("#isDateRange", () => {
    INVALID_DATE_RANGE_VALUES.forEach((value: any[]) => {
      it(`should return ${$v("false")} for ${$c(value, true)}`, () => {
        assert.isFalse(isDateRange(value));
      });
    });

    VALID_DATE_RANGE_VALUES.forEach(value => {
      it(`should return ${$v("true")} for ${$c(value, true)}`, () => {
  ...
  describe("#getSafeDate", () => {
    ...
    forEach(expectations, (expectedResult, value) => {
      it(`should support ${$v(value)}`, () => {
  ...
  describe("ISO_DATE_PATTERN", () => {
    ...
    forEach(expectations, (expectation, value) => {
      it(`should match ${$v(value)}`, () => assert.deepEqual(ISO_DATE_PATTERN.exec(value).slice(1), expectation));
    });
    ...
    invalidDateValues.forEach(value => {
      it(`should *NOT* match ${$v(value)}`, () => assert.isNull(ISO_DATE_PATTERN.exec(value)));
    });
  });
});
