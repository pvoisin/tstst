...
export function getCartesianProduct(...vectors: any[]): any[] {
  return reduce(
    vectors,
    (accumulator, vector) => flatten(map(accumulator, product => map(vector, value => product.concat([value])))),
...
export function isEmpty(value: any): boolean {
  ...
  // Case of collections - leverage `size`:
  if (result === undefined && isCollection(value)) {
    result = _isEmpty(value);
  }
...
export function isCollection(value: any): boolean {
  return isObjectLike(value) || isArrayLike(value) || value instanceof Map || value instanceof Set;
}
...
export function removeMissingValues<T, C extends Collection<T>>(collection: C): C {
  return filter(collection, value => !isNil(value));
}
...
export function filter<T, C extends Collection<T>>(collection: C, predicate: (item: T) => boolean): C {
  ...
  if (isArray(collection)) {
    result = _filter(collection, predicate);
  } else {
    result = pickBy(collection, predicate);
  }
...
export function isMatching(model: any, expectedModel: any, keyFilter?: string[]): boolean {
  if (!_isEmpty(keyFilter)) {
    expectedModel = pick(expectedModel, keyFilter);
  }

  return isMatch(model, expectedModel);
}
 ...
 *
 * @example
 *  • isCompatible({'A': 1, 'B': 2, 'C': 3}, {'A': 1, 'B': isBoolean}) // → false
 *  • isCompatible({'A': 1, 'B': true, 'C': 3}, {'A': 1, 'B': isBoolean}) // → true
 *
 * @param model Model to check.
...
export function isCompatible(model: any, expectedModel: any, keyFilter?: string[]): boolean {
  ...
  forEach(expectedModel, (value, key) => {
    if (_isEmpty(keyFilter) || (keyFilter && key in keyFilter)) {
  ...
  let result = isMatch(model, _expectedModel);
  if (!isCompatibleRulesEmpty) {
    result = result && conformsTo(modelSubSet, compatibilityRules);
  }
